...
resfo_fail:
	kernel sys_write, stdout, resfo_fail_str, resfo_fail_strlen
	kernel sys_exit, 1
...
.start_of_line:
	xor ebp, ebp			; length of the current line
	inc ecx
	cmp byte [buffer+ecx-1], `\n`
	je .new_line
...
.new_line:
	mov dword [current_line_terminated], 1
...
.eof:
	cmp dword [current_line_terminated], 1
	je .quit
	cmp dword [current_line_started], 1
	je .quit
	inc ebx
